...
\paragraph{Objetivo y alcance de las simulaciones}
En comparación con una política puramente miope que ignora $-\psi_H\Delta H_t$, la regla calibrada internaliza el subsidio informacional y reasigna $(\alpha_t,\gamma_t)$ hacia instrumentos que separan verosimilitudes. Los ejercicios DC, PAR, ELN y FARC ilustran tres implicaciones operativas de la teoría: la concentración posterior en horizonte finito en la dirección predicha por el Teorema~\ref{thm:posterior-consistency}, la actualización con soporte completo a lo largo de la senda habilitada por el MDG, y el cumplimiento ex-post de $IC^K$, $IR^K$ e $IR^F$ bajo la lógica de implementabilidad del Teorema~\ref{thm:ebp-implementabilidad}. Estos resultados no afirman la identificación asintótica sobre $\mathcal{T}$, que permanece como un enunciado teórico de límite.
