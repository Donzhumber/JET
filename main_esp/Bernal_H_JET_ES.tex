...
Para evaluar la robustez del mecanismo de identificación, esta sección analiza las simulaciones dinámicas bajo la exclusión contrafactual del canal de comunicación de voz. En la calibración de referencia de la Sección~\ref{sec:calibracion}, la actualización bayesiana del Estado incorpora tanto las señales físicas de riesgos competitivos (desenlaces de maduración y riesgos) como las características acústicas de las llamadas de voz. Aquí, aislamos los canales físico-institucionales estableciendo el indicador de llamada de voz $V_t = 0$ para todos los períodos (o equivalentemente, desactivando la retroalimentación de verosimilitud de la comunicación). Bajo esta configuración, la única fuente de aprendizaje para el Estado consiste en las señales físico-institucionales endógenas generadas por los riesgos competitivos del MDG ($\lambda_j$). El objetivo es verificar si la creencia posterior $\mu_t$ converge al tipo de captor verdadero $\theta_K^\ast$ basándose únicamente en estos desenlaces físico-institucionales, validando que el mecanismo subyacente es autosuficiente para la identificación.
...
Como se muestra en la Figura~\ref{fig:cal-eln-par-mu-novoice}, la creencia posterior del tipo verdadero $\mu_t(\theta_K^\ast)$ converge a la certeza casi absoluta en los cuatro escenarios:
\begin{itemize}
    \item En el \emph{Escenario~I} (DC), la creencia posterior $\mu_t(\mathrm{DC})$ converge desde una actualización posterior al primer período de $0{,}130$ a $0{,}999$ en el período $\tau = 200$.
    \item En el \emph{Escenario~II} (PAR), $\mu_t(\mathrm{PAR})$ asciende de $0{,}344$ a $0{,}997$ en el período $\tau = 200$.
    \item En el \emph{Escenario~III} (ELN), $\mu_t(\mathrm{ELN})$ alcanza $0{,}981$ en el período $\tau = 250$.
    \item En el \emph{Escenario~IV} (FARC), $\mu_t(\mathrm{FARC})$ alcanza $1{,}000$ en el período $\tau = 400$.
\end{itemize}
Estos resultados confirman la predicción teórica del Teorema~\ref{thm:posterior-consistency}: los riesgos competitivos físico-institucionales ($\lambda_j$), que dependen de los coeficientes contingentes al tipo ($\beta_{K,j,\theta}, \zeta_{i,\theta}$) y de los controles del Estado $(\alpha_t, \gamma_t)$, exhiben suficiente variación para separar asintóticamente los tipos. Desactivar el canal de voz no bloquea la identificación, pero ralentiza significativamente la velocidad de convergencia. En el caso de referencia, las características acústicas permitían que el posterior se concentrara dentro de $20$ a $105$ períodos; sin ellas, el Estado requiere una historia más larga de señales físico-institucionales (hasta $400$ períodos en el escenario FARC) para lograr una precisión equivalente. Esto destaca el papel dual de las características de la voz como un acelerador informacional y no como una condición necesaria para la identificación.
...
Las Figuras~\ref{fig:cal-eln-par-gamma-novoice} y~\ref{fig:cal-eln-par-alpha-novoice} presentan las trayectorias de los instrumentos óptimos del Estado bajo exclusión de comunicación de voz. Debido a que el aprendizaje es más lento, el Estado mantiene niveles elevados de presión operativa $\gamma_t^\ast$ e interdicción $\alpha_t^\ast$ durante un mayor número de períodos. Por ejemplo, en el caso DC, la presión promedio es $\bar\gamma^\ast \approx 0{,}573$ y la interdicción promedio es $\bar\alpha^\ast \approx 0{,}825$ a lo largo de la simulación de 200 períodos, en comparación con $\bar\gamma^\ast \approx 0{,}564$ y $\bar\alpha^\ast \approx 0{,}896$ en el caso de referencia. La actualización más lenta significa que el Estado no puede flexibilizar rápidamente los instrumentos hacia sus puntos de referencia de información completa, permaneciendo en una fase prolongada de aprendizaje activo.
...
Las trayectorias conjuntas en el espacio de precisión y política, $(\iota_t,\gamma_t^\ast)$ y $(\iota_t,\alpha_t^\ast)$ (Figuras~\ref{fig:cal-eln-par-iota-gamma-novoice} y~\ref{fig:cal-eln-par-iota-alpha-novoice}), revelan un ascenso mucho más gradual y suave en comparación con el caso de referencia, lo cual es consistente con la ausencia de saltos informacionales repentinos provenientes de las llamadas de voz.
...
Además, los diagnósticos de cumplimiento en la Figura~\ref{fig:cal-eln-par-constraints-novoice} muestran que la viabilidad estratégica del mecanismo se preserva bajo este aprendizaje más lento, con las restricciones $IC^K, IR^K$ e $IR^F$ satisfechas en el 100\% de los períodos simulados. La sensibilidad $\kappa_h(\theta_K^\ast,t)$ se mantiene positiva en todos los escenarios (Figura~\ref{fig:cal-eln-par-kh-novoice}), garantizando que la presión del Estado continúa reduciendo la duración esperada del cautiverio. Los detalles de las simulaciones se resumen en la Tabla~\ref{tab:calibracion-resumen-novoice}.
...
\begin{tabular}{lrrrrrl}
\toprule
$\theta_K^\ast$ & $\mu_1(\theta^\ast)$ & $\mu_{\tau_{\max}}(\theta^\ast)$ & $\bar{\alpha}^\ast$ & $\bar{\gamma}^\ast$ & $\tau_{\max}$ & Desenlace \\
\midrule
DC & 0{,}130 & 0{,}999 & 0{,}825 & 0{,}573 & 200 & Continuar \\
PAR & 0{,}344 & 0{,}997 & 0{,}789 & 0{,}592 & 200 & Continuar \\
ELN & 0{,}361 & 0{,}981 & 0{,}706 & 0{,}653 & 250 & Continuar \\
FARC & 0{,}160 & 1{,}000 & 0{,}757 & 0{,}716 & 400 & Continuar \\
\bottomrule
\end{tabular}
\end{table}
