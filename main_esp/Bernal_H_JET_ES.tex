...
\bottomrule
\end{tabular}
\end{table}En conjunto, el contraste de los cuatro escenarios muestra que el mismo mecanismo bayesiano genera trayectorias de aprendizaje, intensidades de instrumentos, y tasas de viabilidad de restricciones distintas cuando, conjuntamente, cambian el tipo verdadero del captor y los márgenes institucional, familiar, de víctima, y geográfico descritos en el protocolo. Las simulaciones no sustituyen al Teorema~\ref{thm:posterior-consistency}, que requiere separación recurrente en el evento $\mathcal{T}$; ilustran que, con priors y hazards calibrados a la evidencia colombiana, el subsidio entrópico desplaza la política lejos de regímenes puramente miopes y acelera o disciplina la concentración posterior en horizontes finitos, en la dirección predicha por el resultado de identificación.

\clearpage

\section{Simulaciones bajo Exclusión de la Comunicación de Voz}\label{sec:no-voice}

Para evaluar la robustez y suficiencia del mecanismo de identificación, esta sección analiza las simulaciones dinámicas bajo la exclusión contrafactual del canal de comunicación de voz, diferenciándose de la Sección~\ref{sec:calibracion} en que se elimina por completo la información acústica ($V_t = 0$ para todo $t$). En la calibración de referencia de la Sección~\ref{sec:calibracion}, la actualización bayesiana del Estado incorpora tanto las señales físico-institucionales de riesgos competitivos (desenlaces de maduración y riesgos) como las llamadas de voz. Aquí, aislamos los canales físico-institucionales endógenos estableciendo el indicador de llamada de voz $V_t = 0$ para todos los períodos (o equivalentemente, desactivando la retroalimentación de verosimilitud de la comunicación). Bajo esta configuración, la única fuente de aprendizaje para el Estado consiste en las señales físico-institucionales generadas por los riesgos competitivos del MDG ($\lambda_j$), con el objetivo de verificar si la creencia posterior $\mu_t$ converge al tipo de captor verdadero $\theta_K^\ast$ basándose únicamente en estos desenlaces, validando que el mecanismo es autosuficiente para la identificación.

Como se muestra en la Figura~\ref{fig:cal-eln-par-mu-novoice}, la creencia posterior del tipo verdadero $\mu_t(\theta_K^\ast)$ converge a la certeza casi absoluta en los cuatro escenarios bajo exclusión de comunicación de voz. En el \emph{Escenario~I} (DC), la creencia posterior $\mu_t(\mathrm{DC})$ converge desde una actualización posterior al primer período de $0{,}130$ a $0{,}999$ en el período $\tau = 200$. En el \emph{Escenario~II} (PAR), $\mu_t(\mathrm{PAR})$ asciende de $0{,}344$ a $0{,}997$ en el período $\tau = 200$. En el \emph{Escenario~III} (ELN), $\mu_t(\mathrm{ELN})$ alcanza aproximadamente $1{,}000$ en el período $\tau = 300$, mientras que en el \emph{Escenario~IV} (FARC), $\mu_t(\mathrm{FARC})$ alcanza aproximadamente $1{,}000$ en el período $\tau = 400$. Estos resultados confirman la predicción teórica del Teorema~\ref{thm:posterior-consistency}: los riesgos competitivos físico-institucionales ($\lambda_j$), que dependen de los coeficientes contingentes al tipo y de los controles del Estado $(\alpha_t, \gamma_t)$, exhiben suficiente variación para separar asintóticamente los tipos. Desactivar el canal de voz no bloquea la identificación, pero ralentiza significativamente la velocidad de convergencia en comparación con la Sección~\ref{sec:calibracion}. En el caso de referencia, las características acústicas permitían que el posterior se concentrara dentro de $20$ a $105$ períodos; sin ellas, el Estado requiere una historia de señales físico-institucionales sustancialmente más larga, llegando a requerir hasta $400$ períodos en el escenario FARC, para lograr una precisión equivalente. Esto destaca el papel de la voz como un acelerador informacional y no como una condición necesaria para la identificación.
...
\end{subfigure}
\caption{Creencias bayesianas de cada tipo $\mu_\tau(\theta)$ en los diferentes escenarios bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-mu-novoice}
\figurenote{\textit{Nota:} Los paneles (a)-(d) replican los escenarios de referencia pero con el canal de comunicación de voz desactivado. La banda gris marca el límite contrafactual de parada $\tau^{\mathrm{hyp}}$.}
\end{figure}

La Figura~\ref{fig:cal-eln-par-gamma-novoice} presenta la trayectoria de la presión operativa óptima $\gamma_t^\ast$ y su comparación con el benchmark de información completa $\gamma^R$. Debido a que el aprendizaje es más lento sin la retroalimentación de la voz, el Estado se ve obligado a mantener niveles elevados de presión operativa durante un mayor número de períodos para inducir separación y forzar la revelación a través del hazard de rescate. Por ejemplo, en el caso DC, la presión promedio es $\bar\gamma^\ast \approx 0{,}573$ a lo largo de la simulación de 200 períodos, en comparación con $\bar\gamma^\ast \approx 0{,}564$ en la Sección~\ref{sec:calibracion}, permaneciendo en una fase prolongada de aprendizaje activo antes de flexibilizarse hacia el punto de referencia de información completa correspondiente al tipo verdadero.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario IV: FARC (sin voz)}
\end{subfigure}
\caption{Presión operativa $\gamma_t^\ast$ y punto de referencia $\gamma^R$ frente a $\tau$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-gamma-novoice}
\end{figure}

De manera similar, la Figura~\ref{fig:cal-eln-par-alpha-novoice} ilustra la trayectoria de la interdicción financiera óptima $\alpha_t^\ast$ y su comparación con el punto de referencia de información completa $\alpha^R$. Al igual que con la presión operativa, la interdicción óptima se mantiene elevada durante más períodos en todos los escenarios bajo exclusión de voz. Por ejemplo, en el Escenario I (DC), el promedio de interdicción a lo largo de la simulación de 200 períodos es $\bar\alpha^\ast \approx 0{,}825$ (comparado con $0{,}896$ en los 35 períodos del caso con voz). Aunque el promedio sobre todo el horizonte es menor debido al retiro gradual del instrumento tras la convergencia tardía, en la fase inicial de la interacción el Estado sostiene la interdicción en niveles elevados por más tiempo para compensar la falta de señales acústicas y evitar la negociación prematura.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario IV: FARC (sin voz)}
\end{subfigure}
\caption{Interdicción financiera $\alpha_t^\ast$ y punto de referencia $\alpha^R$ frente a $\tau$ bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-alpha-novoice}
\end{figure}

La Figura~\ref{fig:cal-eln-par-iota-gamma-novoice} analiza las trayectorias de precisión posterior y presión operativa en el espacio conjunto $(\iota_t,\gamma_t^\ast)$. A diferencia de la Sección~\ref{sec:calibracion}, donde la retroalimentación de voz generaba saltos informacionales marcados en los primeros períodos, la precisión posterior asciende aquí de manera continua y gradual en todos los escenarios. Esta suavidad en el incremento de la precisión refleja que el aprendizaje descansa exclusivamente en el goteo constante de las tasas de hazard del MDG. En consecuencia, la presión operativa se reduce gradualmente solo a medida que la precisión converge a su límite superior, ilustrando un acoplamiento estrecho y sin saltos discretos entre la acumulación de información física y el retiro del instrumento.
...
La Figura~\ref{fig:cal-eln-par-iota-alpha-novoice} muestra las trayectorias conjuntas de precisión posterior e interdicción financiera $(\iota_t,\alpha_t^\ast)$. En correspondencia con la trayectoria de la presión, el retiro de la interdicción financiera hacia la rama de información completa óptima es mucho más lento y suave. La ausencia del subsidio informacional acústico elimina las transiciones rápidas de política, forzando al Estado a sostener la interdicción financiera en niveles altos para mantener la compatibilidad de incentivos (evitando la capitulación) mientras la creencia se actualiza lentamente período a período a partir de los desenlaces físicos.
...
El diagnóstico de restricciones en la Figura~\ref{fig:cal-eln-par-constraints-novoice} confirma que la viabilidad estratégica del mecanismo se preserva al 100\,\% en todos los ciclos simulados, incluso bajo las condiciones de actualización lenta por exclusión de voz. Las restricciones de racionalidad individual de la familia y el captor ($IR^F, IR^K$), así como la restricción de compatibilidad de incentivos del captor ($IC^K$), se satisfacen sin excepción. Esto demuestra que la formulación de control dual del Estado es robusta a cambios en la velocidad del aprendizaje, ajustando las trayectorias de política para asegurar que las restricciones de participación y de incentivos de equilibrio sigan siendo viables en cada período.
...
La Figura~\ref{fig:cal-eln-par-kh-novoice} traza la sensibilidad neta de la duración esperada del cautiverio $\kappa_h(\theta_K^\ast,t)$ respecto a la presión operativa. A diferencia de las fluctuaciones observadas bajo información con voz, al desactivar la comunicación acústica la sensibilidad neta se mantiene positiva ($\kappa_h > 0$) en todos los escenarios simulados, lo que implica $\partial\,\mathbb{E}[\tau\mid\theta_K]/\partial\gamma_t^\ast < 0$. Este resultado sugiere que, ante la ausencia del canal de voz que aceleraba la convergencia, el Estado debe depender críticamente del hazard de rescate físico para influir en las decisiones del captor, haciendo que la presión operativa reduzca la duración esperada del cautiverio en el 100\,\% de los ciclos simulados sin que el efecto desincentivo de la disuasión llegue a prolongar el secuestro.
...
La Figura~\ref{fig:cal-eln-par-outcomes-novoice} ilustra la evolución de las tasas de hazard de maduración y riesgos del secuestrador a lo largo de las trayectorias simuladas. El escalamiento de las tasas de maduración $\lambda_M$ y de los riesgos de cierre no regulado refleja cómo las decisiones estratégicas de los captores responden dinámicamente a la presión prolongada del Estado. A medida que la creencia bayesiana converge al tipo verdadero, la estabilización de los hazards alrededor de sus benchmarks óptimos demuestra que el comportamiento del secuestrador se disciplina hacia los desenlaces esperados de información completa, a pesar del retraso temporal inducido por la falta de señales de voz.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Escenario IV: FARC (sin voz)}
\end{subfigure}
\caption{Escalamiento de maduración e indicadores de riesgo bajo exclusión de comunicación de voz.}
\label{fig:cal-eln-par-outcomes-novoice}
\end{figure}

Los resultados cuantitativos de estas simulaciones contrafactuales se resumen en la Tabla~\ref{tab:calibracion-resumen-novoice}. En todos los escenarios, la creencia posterior converge a la certeza casi absoluta ($\mu_{\tau_{\max}}(\theta^\ast) \ge 0{,}997$), requiriendo horizontes máximos $\tau_{\max}$ que van desde 200 períodos en DC y PAR hasta 400 períodos en FARC. En conjunto, la comparación con la Sección~\ref{sec:calibracion} resalta que el mecanismo bayesiano es robusto y autosuficiente para la identificación, operando eficazmente incluso cuando la pérdida del canal de comunicación acústica disminuye la velocidad de aprendizaje del Estado.
...
$\theta_K^\ast$ & $\mu_0(\theta^\ast)$ & $\mu_{\tau_{\max}}(\theta^\ast)$ & $\bar{\alpha}^\ast$ & $\bar{\gamma}^\ast$ & $\tau^{\mathrm{hyp}}$ & $m_{\tau^{\mathrm{hyp}}}$ \\
\midrule
DC & 0.350 & 0.999 & 0.825 & 0.573 & 200 & Continuar \\
PAR & 0.150 & 0.997 & 0.789 & 0.592 & 200 & Continuar \\
ELN & 0.250 & 1.000 & 0.784 & 0.673 & 300 & Continuar \\
FARC & 0.150 & 1.000 & 0.757 & 0.716 & 400 & Continuar \\
\bottomrule
\end{tabular}
\end{table}
