% !TEX root = Appendix_B_Micro.tex
% =========================================================================
% APPENDIX B -- MICROSTRUCTURE AND COMPETING HAZARDS IN COLOMBIA
...
Appendix B: Microstructure and Competing Hazards in Colombia}
\author{Humberto Bernal\thanks{Economist, professor of the Economics Program at the Universidad Colegio Mayor de Cundinamarca and graduate of the Ph.D.\ in Economics from the Universidad de los Andes, Colombia. E-mail: \href{mailto:hbernal@uniandes.edu.co}{hbernal@uniandes.edu.co}, \href{mailto:hbernalc@universidadmayor.edu.co}{hbernalc@universidadmayor.edu.co}. ORCID: 0000-0002-4864-1726.}}
\date{\today}
\maketitle
...
the \texttt{Appendix\_2/} replication package. Run \path{replicate.do} from any
working directory---it locates \path{run_all.do} automatically without a manual
\texttt{cd}. See \path{README.txt} for step-by-step instructions.
...
\begin{sloppypar}
\begin{enumerate}
\item \path{replicate.do} auto-detects the package root and delegates to
  \path{stata/run_all.do}, which runs \path{microstructure_replication.do},
  \path{sample_selection_robustness.do}, and \path{cox_only.do}
  in sequence; all output is saved to \path{stata/outputs/replication.log}.
\item Each script includes \path{_setup_paths.do} and locates
  \path{stata/data/Data_merge.dta} automatically.
\item Key outputs written to \path{stata/outputs/} include
...
\item (Optional) Compile the PDF: run \texttt{pdflatex} and \texttt{bibtex}
  from \path{tex/}.
\end{enumerate}
\end{sloppypar}
