...
\paragraph{Objective and scope of the simulations.}
Relative to a purely myopic policy that ignores $-\psi_H\Delta H_t$, the calibrated rule internalizes the informational subsidy and reallocates $(\alpha_t,\gamma_t)$ toward instruments that separate likelihoods. The DC, PAR, ELN, and FARC exercises illustrate three operational implications of the theory: finite-horizon posterior concentration in the direction predicted by Theorem~\ref{thm:posterior-consistency}, MDG-enabled full-support updating along the path, and ex-post compliance with $IC^K$, $IR^K$, and $IR^F$ under the implementability logic of Theorem~\ref{thm:ebp-implementabilidad}. These results do not claim asymptotic identification on $\mathcal{T}$, which remains a theoretical limit statement.
