...
These four theoretical contributions are then subjected to applied validation in Sections~\ref{sec:application}--\ref{sec:calibracion}, which map the abstract environment to kidnapping for ransom in Colombia. Multinomial logit and competing-risks estimates from National Center for Historical Memory microdata supply type-specific hazards; DC, PAR, ELN, and FARC calibrations then report posterior paths, instrument trajectories, informational gain, and IR/IC compliance. The application thus constitutes a validation layer, not the theoretical primitive.
...
\figurenote{\textit{Note:} Panel (a) presents the optimal financial interdiction in Scenario~I (DC). Panel (b) presents Scenario~II (PAR). Panel (c) presents Scenario~III (ELN). Panel (d) presents Scenario~IV (FARC). The dashed lines correspond to the Rescue ($\alpha_R^\mu$) and Negotiation ($\alpha_N^\mu$) belief-weighted benchmarks. The gray band marks $\tau^{\mathrm{hyp}}$.}
\end{figure}
\vspace{-6pt}
...
Taken together, the four-scenario contrast shows that the same Bayesian mechanism generates distinct learning trajectories, instrument intensities, and constraint viability rates when, jointly, the captor's true type and the institutional, family, victim, and geographic margins described in the protocol change. The simulations do not substitute for Theorem~\ref{thm:posterior-consistency}, which requires recurrent separation in the event $\mathcal{T}$; they illustrate that, with priors and hazards calibrated to Colombian evidence, the entropic subsidy shifts policy away from purely myopic regimes and accelerates or disciplines posterior concentration in finite horizons, in the direction predicted by the identification result.

\section{Simulations under Excluded Voice Communication}\label{sec:no-voice}

To evaluate the robustness of the identification mechanism, this section analyzes the dynamic simulations under the counterfactual exclusion of the voice communication channel. In the baseline calibration of Section~\ref{sec:calibracion}, the State's Bayesian update incorporates both the physical competing-risks signals (maturation and hazard outcomes) and the acoustic features of voice calls. Here, we isolate the physical-institutional channels by setting the voice call indicator $V_t = 0$ for all periods (or equivalently, disabling the communication likelihood feedback). Under this setup, the only source of learning for the State consists of the endogenous physical-institutional signals, competing hazards ($\lambda_j$) and collusion detection ($d_t$), generated after the MDG filter. The objective is to verify whether the posterior belief $\mu_t$ converges to the true captor type $\theta_K^\ast$ solely based on these physical-institutional outcomes, validating that the underlying mechanism is self-sufficient for identification.
...
\figurenote{\textit{Note:} Panels (a)-(d) replicate the baseline scenarios but with the voice communication channel deactivated. The gray band marks the counterfactual stopping boundary $\tau^{\mathrm{hyp}}$.}
\end{figure}

As shown in Figure~\ref{fig:cal-eln-par-mu-novoice}, the posterior belief of the true type $\mu_t(\theta_K^\ast)$ converges to near-certainty in all four scenarios:
\begin{itemize}
    \item In \emph{Scenario~I} (DC), the posterior belief $\mu_t(\mathrm{DC})$ converges from a post-first-period update of $0.130$ to $0.999$ by period $\tau = 150$.
    \item In \emph{Scenario~II} (PAR), $\mu_t(\mathrm{PAR})$ climbs from $0.344$ to $0.989$ by period $\tau = 150$.
    \item In \emph{Scenario~III} (ELN), $\mu_t(\mathrm{ELN})$ reaches $1.000$ by period $\tau = 300$.
    \item In \emph{Scenario~IV} (FARC), $\mu_t(\mathrm{FARC})$ reaches $1.000$ by period $\tau = 400$.
\end{itemize}
These results confirm the theoretical prediction of Theorem~\ref{thm:posterior-consistency}: the physical-institutional competing hazards ($\lambda_j$), which depend on type-contingent coefficients ($\beta_{K,j,\theta}, \zeta_{i,\theta}$) and the State's controls $(\alpha_t, \gamma_t)$, exhibit sufficient variation to asymptotically separate the types. Shutting down the voice channel does not block identification, but it significantly slows down the speed of convergence. In the baseline case, the acoustic features allowed the posterior to concentrate within $20$ to $105$ periods; without them, the State requires a longer history of physical-institutional signals (up to $400$ periods in the FARC scenario) to achieve equivalent precision. This highlights the dual role of voice features as an informational accelerator rather than a necessary condition for identification.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Operative pressure $\gamma_t^\ast$ and benchmarks $\gamma_R^\mu$ and $\gamma_N^\mu$ against $\tau$ under excluded voice communication.}
\label{fig:cal-eln-par-gamma-novoice}
\figurenote{\textit{Note:} Panels (a)-(d) replicate the baseline scenarios but with the voice communication channel deactivated. The dashed lines correspond to the Rescue ($\gamma_R^\mu$) and Negotiation ($\gamma_N^\mu$) belief-weighted benchmarks. The gray band marks the counterfactual stopping boundary $\tau^{\mathrm{hyp}}$.}
\end{figure}
...
\figurenote{\textit{Note:} Panels (a)-(d) replicate the baseline scenarios but with the voice communication channel deactivated. The dashed lines correspond to the Rescue ($\alpha_R^\mu$) and Negotiation ($\alpha_N^\mu$) belief-weighted benchmarks. The gray band marks the counterfactual stopping boundary $\tau^{\mathrm{hyp}}$.}
\end{figure}

Under the counterfactual exclusion of voice communication, the trajectory of the optimal operational pressure $\gamma_t^\ast$ and its comparison with the $\gamma_R^\mu$ and $\gamma_N^\mu$ benchmarks, detailed in Figure~\ref{fig:cal-eln-par-gamma-novoice}, reflect the impact of slower learning on instrument persistence. Because the acquisition of information is slower without acoustic feedback, the State is forced to prolong high levels of operational pressure for a larger number of periods to induce separation and force revelation through the rescue hazard. For instance, in the DC scenario, the average pressure is $\bar\gamma^\ast \approx 0.577$ across the 150 simulated periods, compared to the average of $\bar\gamma^\ast \approx 0.564$ in the baseline calibration with voice (Section~\ref{sec:calibracion}). Just as in the baseline, the optimal policy fluctuates between both references under dual control before converging toward the full-information benchmark of the executed branch (Negotiation) once uncertainty is resolved, although the absence of voice signals significantly extends the active-learning phase.

In a similar direction, the trajectory of the optimal financial interdiction $\alpha_t^\ast$ relative to the $\alpha_R^\mu$ and $\alpha_N^\mu$ benchmarks, shown in Figure~\ref{fig:cal-eln-par-alpha-novoice}, demonstrates an equivalent persistence when communication is absent. As with operational pressure, the State is forced to sustain interdiction at elevated levels for a larger absolute number of periods across all scenarios under voice exclusion, compensating for the lack of acoustic signals and preventing premature negotiation. For instance, in Scenario I of DC this greater absolute persistence does not translate into a higher average: the gradual withdrawal of the instrument after late convergence extends the relevant horizon out to the full 150 simulated periods, yielding an average of $\bar\alpha^\ast \approx 0.827$; the baseline case with voice, by contrast, converges within just 35 periods and records a higher average of $0.896$, precisely because that shorter window never reaches the decline phase that follows convergence.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Posterior accuracy and operative pressure path $(\iota_t,\gamma_t^\ast)$ under excluded voice communication.}
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Posterior accuracy and financial interdiction path $(\iota_t,\alpha_t^\ast)$ under excluded voice communication.}
\label{fig:cal-eln-par-iota-alpha-novoice}
\end{figure}

The joint trajectories in the space of accuracy and policy, $(\iota_t,\gamma_t^\ast)$ and $(\iota_t,\alpha_t^\ast)$ (Figures~\ref{fig:cal-eln-par-iota-gamma-novoice} and~\ref{fig:cal-eln-par-iota-alpha-novoice}), reveal a much more gradual and smooth ascent compared to the baseline, which is consistent with the absence of sudden informational jumps from calls.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Net sensitivity $\kappa_h(\theta_K^\ast,t)$ of expected captivity duration under excluded voice communication.}
\label{fig:cal-eln-par-kh-novoice}
\end{figure}

Furthermore, the compliance diagnostics in Figure~\ref{fig:cal-eln-par-constraints-novoice} show that the strategic feasibility of the mechanism is preserved under this slower learning path, with $IC^K, IR^K$, and $IR^F$ satisfied in 100\% of the simulated periods. Without acoustic signals to accelerate convergence, the expected informational gain $\Delta H_t$ (Figure~\ref{fig:cal-eln-par-dh-novoice}) remains positive over a substantially longer horizon than in Section~\ref{sec:calibracion}, reflecting the State's continued reliance on the physical-institutional channel alone. The net sensitivity $\kappa_h(\theta_K^\ast,t)$ (Figure~\ref{fig:cal-eln-par-kh-novoice}) reproduces the same heterogeneous pattern found in Section~\ref{sec:calibracion}: in Scenario~II (PAR) and Scenario~IV (FARC), $\kappa_h>0$ in $87.3\,\%$ and $91.3\,\%$ of cycles, respectively, so that higher operational pressure shortens expected captivity; in Scenario~I (DC), the sensitivity is again predominantly negative, with $\kappa_h<0$ in $74.7\,\%$ of periods, so that pressure instead tends to prolong captivity; and in Scenario~III (ELN) the sign splits almost evenly between both regimes, with $\kappa_h>0$ in $50.0\,\%$ of periods. This confirms that the strategic trade-off between operational pressure and expected captivity duration identified in the baseline calibration persists essentially unchanged even without the voice channel. The simulation details are summarized in Table~\ref{tab:calibracion-resumen-novoice}.
...
\begin{tabular}{lrrrrrl}
\toprule
$\theta_K^\ast$ & $\mu_1(\theta^\ast)$ & $\mu_{\tau_{\max}}(\theta^\ast)$ & $\bar{\alpha}^\ast$ & $\bar{\gamma}^\ast$ & $\tau_{\max}$ & Outcome \\
\midrule
DC & 0.130 & 0.999 & 0.827 & 0.577 & 150 & Continue \\
PAR & 0.344 & 0.989 & 0.852 & 0.616 & 150 & Continue \\
ELN & 0.070 & 1.000 & 0.784 & 0.673 & 300 & Continue \\
FARC & 0.160 & 1.000 & 0.757 & 0.716 & 400 & Continue \\
\bottomrule
\end{tabular}
\end{table}
...
At the applied level, Colombian kidnapping for ransom maps the abstract type space into estimated criminal technologies. Competing-risks and multinomial evidence discipline hazards and priors; four-scenario simulations show that the calibrated dual-control rule accelerates posterior concentration, sustains high but disciplined instruments, and preserves IR/IC feasibility, in the direction predicted by the theorems without substituting for them.
