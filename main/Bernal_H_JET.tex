...
To evaluate the robustness of the identification mechanism, this section analyzes the dynamic simulations under the counterfactual exclusion of the voice communication channel. In the baseline calibration of Section~\ref{sec:calibracion}, the State's Bayesian update incorporates both the physical competing-risks signals (maturation and hazard outcomes) and the acoustic features of voice calls. Here, we isolate the physical-institutional channels by setting the voice call indicator $V_t = 0$ for all periods (or equivalently, disabling the communication likelihood feedback). Under this setup, the only source of learning for the State consists of the endogenous physical-institutional signals generated by the MDG competing hazards ($\lambda_j$). The objective is to verify whether the posterior belief $\mu_t$ converges to the true captor type $\theta_K^\ast$ solely based on these physical-institutional outcomes, validating that the underlying mechanism is self-sufficient for identification.
...
\begin{itemize}
    \item In \emph{Scenario~I} (DC), the posterior belief $\mu_t(\mathrm{DC})$ converges from a post-first-period update of $0.130$ to $0.999$ by period $\tau = 200$.
    \item In \emph{Scenario~II} (PAR), $\mu_t(\mathrm{PAR})$ climbs from $0.344$ to $0.997$ by period $\tau = 200$.
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Operative pressure $\gamma_t^\ast$ and reference point $\gamma^R$ against $\tau$ under excluded voice communication.}
...
\begin{subfigure}[t]{0.48\linewidth}
  ...
  \caption{Scenario IV: FARC (no voice)}
\end{subfigure}
\caption{Financial interdiction $\alpha_t^\ast$ and reference point $\alpha^R$ against $\tau$ under excluded voice communication.}
\label{fig:cal-eln-par-alpha-novoice}
\end{figure}

Figures~\ref{fig:cal-eln-par-gamma-novoice} and~\ref{fig:cal-eln-par-alpha-novoice} present the trajectories of the State's optimal instruments under excluded voice communication. Because learning is slower, the State maintains high levels of operative pressure $\gamma_t^\ast$ and interdiction $\alpha_t^\ast$ for a larger number of periods. For instance, in the DC case, the average pressure is $\bar\gamma^\ast \approx 0.573$ and interdiction is $\bar\alpha^\ast \approx 0.825$ across the 200-period simulation, compared to $\bar\gamma^\ast \approx 0.564$ and $\bar\alpha^\ast \approx 0.896$ in the baseline. The slower update means the State cannot quickly ease the instruments toward their full-information benchmarks, remaining in a prolonged active-learning phase.
...
\midrule
DC & 0.130 & 0.999 & 0.825 & 0.573 & 200 & Continue \\
PAR & 0.344 & 0.997 & 0.789 & 0.592 & 200 & Continue \\
